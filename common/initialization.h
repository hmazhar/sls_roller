#ifndef CHRONOMODELS_INIT_H
#define CHRONOMODELS_INIT_H
#include "common.h"

void InitObject(ChSharedBodyPtr &body, double mass, ChVector<> pos, ChQuaternion<> rot, ChSharedPtr<ChMaterialSurface> &material, bool do_collide, bool is_fixed, int collision_family,
...
void AddCollisionGeometry(ChSharedBodyPtr &body, ShapeType type, ChVector<> dimension, ChVector<> local_position, ChQuaternion<> local_rotation) {
	...
	if (type == SPHERE) {
		...
		cone_shape->GetConeGeometry().center = local_position;
