...
real3 ComputeMomentOffset_x(real3 tensor, real mass, real disp) {
	real3 output;

	output.x = tensor.x;
	output.y = tensor.y + mass * disp * disp;
	output.z = tensor.z + mass * disp * disp;

	return output;
}

real3 ComputeMomentOffset_y(real3 tensor, real mass, real disp) {
	real3 output;

	output.x = tensor.x + mass * disp * disp;
	output.y = tensor.y;
	output.z = tensor.z + mass * disp * disp;
...
real3 ComputeMomentOffset_z(real3 tensor, real mass, real disp) {
	real3 output;
			...
			for (int j = 0; j < num_per_dir.y; j++) {
				for (int k = 0; k < num_per_dir.z; k++) {
					...
					if (mixture[mix_type] == MIX_SPHERE) {
						...
						real3 local_inertia = R3(2 / 5.0 * mass * r.x * r.x, 2 / 5.0 * mass * r.x * r.x, 2 / 5.0 * mass * r.x * r.x);

						real3 I1 = ComputeMomentOffset_x(local_inertia, mass / 2, -r.x / 3.0);
						real3 I2 = ComputeMomentOffset_x(local_inertia, mass / 2, r.x / 3.0);
						local_inertia = I1 + I2;

						body->SetInertiaXX(ChVector<>(local_inertia.x, local_inertia.y, local_inertia.z));

					} else if (mixture[mix_type] == MIX_ELLIPSOID) {
						AddCollisionGeometry(body, ELLIPSOID, ChVector<>(r.x, r.y, r.z), Vector(0, 0, 0), Quaternion(1, 0, 0, 0));
						if (useGPU == false) {
							body->SetInertiaXX(
									ChVector<>(1 / 5.0 * mass * (r.y * r.y + r.z * r.z), 1 / 5.0 * mass * (r.x * r.x + r.z * r.z), 1 / 5.0 * mass * (r.x * r.x + r.y * r.y)));
						}
					} else if (mixture[mix_type] == MIX_CUBE) {
						AddCollisionGeometry(body, BOX, ChVector<>(r.x, r.y, r.z), Vector(0, 0, 0), Quaternion(1, 0, 0, 0));
						if (useGPU == false) {
							body->SetInertiaXX(
									ChVector<>(1 / 12.0 * mass * (r.y * r.y + r.z * r.z), 1 / 12.0 * mass * (r.x * r.x + r.z * r.z), 1 / 12.0 * mass * (r.x * r.x + r.y * r.y)));
						}
						...
						if (useGPU == false) {
							body->SetInertiaXX(
									ChVector<>(1 / 12.0 * mass * (3 * r.x * r.x + r.y * r.y), 1 / 2.0 * mass * (r.x * r.x), 1 / 12.0 * mass * (3 * r.x * r.x + r.y * r.y)));
						}
						...
						AddCollisionGeometry(body, SPHERE, ChVector<>(r.x * 1.33, r.y * 1.33, r.z * 1.33), Vector(r.x / 4.0, 0, 0), Quaternion(1, 0, 0, 0));
						real3 local_inertia;

						real3 r1 = R3(r.x, r.y, r.z);
						real mass1 = density * 4.0 / 3.0 * PI * r.x * r.x * r.x;
						real3 local_inertia1 = R3(2 / 5.0 * mass1 * r1.x * r1.x, 2 / 5.0 * mass1 * r1.x * r1.x, 2 / 5.0 * mass1 * r1.x * r1.x);

						real3 r2 = R3(r.x * 1.33, r.y * 1.33, r.z * 1.33);
						real mass2 = density * 4.0 / 3.0 * PI * r2.x * r2.x * r2.x;
						real3 local_inertia2 = R3(2 / 5.0 * mass2 * r2.x * r2.x, 2 / 5.0 * mass2 * r2.x * r2.x, 2 / 5.0 * mass2 * r2.x * r2.x);

						real3 I1 = ComputeMomentOffset_x(local_inertia1, mass1, -r.x / 4.0);
						real3 I2 = ComputeMomentOffset_x(local_inertia2, mass2, r.x / 4.0);
						local_inertia = I1 + I2;
						...
						real3 r1 = R3(r.x * .75, r.y, r.z);
						real mass1 = density * 4.0 / 3.0 * PI * r.x * r.x * r.x;
						real3 local_inertia1 = R3(2 / 5.0 * mass1 * r1.x * r1.x, 2 / 5.0 * mass1 * r1.x * r1.x, 2 / 5.0 * mass1 * r1.x * r1.x);

						real3 r2 = R3(r.x * .75, r.y * .5, r.z * .5);
						real mass2 = density * 4.0 / 3.0 * PI * r2.x * r2.y * r2.z;
						real3 local_inertia2 = R3(
								1 / 5.0 * mass2 * (r2.y * r2.y + r2.z * r2.z),
								1 / 5.0 * mass2 * (r2.x * r2.x + r2.z * r2.z),
								1 / 5.0 * mass2 * (r2.x * r2.x + r2.y * r2.y));

						real3 I1 = ComputeMomentOffset_x(local_inertia1, mass1, -r.x / 3.0);
						real3 I2 = ComputeMomentOffset_x(local_inertia2, mass2, r.x / 3.0);
						local_inertia = I1 + I2;
...
//							AddCollisionGeometry(body, ELLIPSOID, ChVector<>(r.x * .75, r.y * .75, r.z * .75), Vector(0, r.x, 0), Quaternion(1, 0, 0, 0));
						...
						real3 I1 = ComputeMomentOffset_x(local_inertia1, mass1, 0);
						real3 I2 = ComputeMomentOffset_x(local_inertia2, mass2, 0);
						local_inertia = I1 + I2;
						...
						real3 r1 = R3(r.x, r.y, r.z) * .75;
						real mass1 = density * 4.0 / 3.0 * PI * r.x * r.x * r.x;
						real3 local_inertia1 = R3(2 / 5.0 * mass1 * r1.x * r1.x, 2 / 5.0 * mass1 * r1.x * r1.x, 2 / 5.0 * mass1 * r1.x * r1.x);

						real3 r2 = R3(r.x, r.y, r.z) * .75;
						real mass2 = density * 4.0 / 3.0 * PI * r.x * r.x * r.x;
						real3 local_inertia2 = R3(2 / 5.0 * mass2 * r2.x * r2.x, 2 / 5.0 * mass2 * r2.x * r2.x, 2 / 5.0 * mass2 * r2.x * r2.x);

						real3 r3 = R3(r.x * .5, r.y, r.z);
						real mass3 = density * 4.0 / 3.0 * PI * r2.x * r2.y * r2.z;
						real3 local_inertia3 = R3(
								1 / 5.0 * mass3 * (r3.y * r3.y + r3.z * r3.z),
								1 / 5.0 * mass3 * (r3.x * r3.x + r3.z * r3.z),
								1 / 5.0 * mass3 * (r3.x * r3.x + r3.y * r3.y));

						real3 I1 = ComputeMomentOffset_x(local_inertia1, mass1, -r.x / 3.0);
						real3 I2 = ComputeMomentOffset_x(local_inertia2, mass2, r.x / 3.0);
						real3 I3 = ComputeMomentOffset_x(local_inertia3, mass3, 0);
						local_inertia = I1 + I2 + I3;
